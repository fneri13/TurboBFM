...
with open(sol_dir + '/' + pik_files[-1], 'rb') as file:
    sol = pickle.load(file)
...
contour = ax.contourf(sol['X'][:, :, 0], sol['Y'][:, :, 0], u, cmap='coolwarm', levels=20)
cbar = fig.colorbar(contour, ax=ax)
ax.quiver(sol['X'][:, :, 0], sol['Y'][:, :, 0], ux, uy)
ax.set_aspect('equal')
ax.set_xlabel(r'$x$')
ax.set_ylabel(r'$y$')


res_names = ['rho', 'rhoU', 'rhoV', 'rhoW', 'rhoE']
plt.figure()
for i in range(len(res_names)):
    plt.plot(sol['Res'][i], label=res_names[i])
plt.legend()
