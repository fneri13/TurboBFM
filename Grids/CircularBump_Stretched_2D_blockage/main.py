...
# GENERATE THE MERIDIONAL GRID AND THE BLOCKAGE GRID
x = X[:,0]
y = np.linspace(0, SPAN, NZ)
Xnew, Ynew = np.meshgrid(x, y, indexing='ij')
...
grid = {"X": Xnew, "Y": Ynew}
with open(OUTPUT_FOLDER + '/grid_%02i_%02i.pik' %(NX, NZ), 'wb') as file:
    pickle.dump(grid, file)

B = np.zeros_like(Xnew)
for j in range(NZ):
    B[:,j] = block

grid = {"Blockage": B}
with open(OUTPUT_FOLDER + '/blockage_%02i_%02i.pik' %(NX, NZ), 'wb') as file:
    pickle.dump(grid, file)
